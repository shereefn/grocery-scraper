...
TEST_STORES = ["LULU Hypermarket", "Hyper Panda", "Othaim Markets", "Nesto", "eXtra", "Danube", "Mark & Save", "Grand Hyper", "Hyper Al Wafa", "Al Madina Hypermarket", "Jarir Bookstore","SACO","Home Box","Carrefour","City Flower","Tamimi Market","City Flower"]
TARGET_PRODUCTS = [] 
...
def check_alerts_and_send_email(products: List[Dict]):
    ...
    html_body += f'''
        <div style="text-align: center; margin-top: 30px; margin-bottom: 30px;">
            <a href="https://riyadh.godeals.me/" style="display: inline-block; padding: 14px 28px; background-color: #1a73e8; color: #ffffff !important; text-decoration: none; border-radius: 8px; font-weight: bold; font-family: Arial, sans-serif; font-size: 16px;">View All Deals on Website</a>
        </div>
    </body>
    </html>
    '''
...
<title>GoDeals | Grocery Offers</title>
...
<script>
  ...
  function loadMore() {{
    ...
    chunk.forEach(item => {{
      ...
      let displayStore = storeArr.join(" & ");
      ...
      card.innerHTML = `
          ...
          <div class="card-store">${{displayStore}}</div>
