  ...
  function applyFilters() {{
    ...
    filteredData = rawData.filter(item => {{
     ...
     const matchStore = selectedStores.length === 0 || selectedStores.some(selected => {{
          ...
          const storeUp = item.Store.toUpperCase();
          
          // Special rule for our combined checkbox
          if (selected === "MARK & SAVE") {{
              return storeUp.includes("MARK") || storeUp.includes("SAVE");
          }}
          
          // Standard rule for everything else
          return storeUp.includes(selected.toUpperCase());
      }});
